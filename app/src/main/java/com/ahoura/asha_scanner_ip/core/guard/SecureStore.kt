...
object SecureStore {
    ...
    // is scoped per app UID, so com.ahoura.asha_scanner_ip.core.guard starts with an empty keystore and
    // the alias is generated fresh on first use. There is nothing to migrate.
    private const val KEY_ALIAS = "asha_guard_credential_key"
    ...
    private fun getOrCreateKey(): SecretKey? = runCatching {
        ...
        if (keyStore.containsAlias(KEY_ALIAS)) {
            ...
            if (entry != null) return@runCatching entry.secretKey
        }
    ...
    fun decrypt(encryptedBase64: String): String {
        ...
        return try {
            ...
            val key = getOrCreateKey() ?: return encryptedBase64
            val cipher = Cipher.getInstance("AES/GCM/NoPadding")
    ...
    fun getSecret(context: Context, key: String, fallback: String = ""): String {
        return runCatching {
            ...
            if (encrypted != null) {
                return@runCatching decrypt(encrypted).ifBlank { fallback }
            }
