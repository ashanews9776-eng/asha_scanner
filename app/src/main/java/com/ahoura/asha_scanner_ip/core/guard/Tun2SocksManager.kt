    ...
    fun start(tunFd: ParcelFileDescriptor, socksProxyPort: Int, dnsOnlyUdpgw: Boolean = false): Boolean {
        ...
        try {
            Tun2SocksJniLoader.initializeLogger(
                "com.ahoura.asha_scanner_ip.core.guard.Tun2SocksManager",
                "logTun2Socks",
            )
        } catch (e: Throwable) {
        ...
        if (!Tun2SocksJniLoader.isLoaded) {
            ConnectionLog.record("tun2socks: libtun2socks.so is not loaded on this device")
            return false
        }
