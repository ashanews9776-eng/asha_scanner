...
object CoreConfig {
     ...
     * absent. See [GuardVpnService.raiseOuterLeg].
     *
     * Written only after the byte threshold in
     * [GuardVpnService.recordWorkingPlainTransport] — a handshake is not
     ...
     * [GuardVpnService.armRegionPhase]. A plain Psiphon connect always takes
     ...
     * the dead `if (!psiphonVpnMode)` arm in `GuardVpnService.onConnected` is what
    ...
    fun portRejection(port: Int): String? = when {
        ...
        port in RESERVED_PORTS -> "$port is used internally by Asha Guard"
        else -> null
    }
