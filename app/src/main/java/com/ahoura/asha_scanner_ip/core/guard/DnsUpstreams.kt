 ...
 * can only speak plain UDP. Before this each of those front-ends carried its
...
object DnsUpstreams {
    ...
    fun list(context: Context): List<String> {
        val raw = context.getSharedPreferences("settings", Context.MODE_PRIVATE).getString(PREF, null)
            ?.split(',', ';', ' ', '\n')
            ?.map(String::trim)
