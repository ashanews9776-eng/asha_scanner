...
object ShardManager {
    ...
    private fun unpackGeoAssets(context: Context, dir: File): Boolean {
        ...
        val assetSiteLen = runCatching { context.assets.open("geosite.dat").use { it.available().toLong() } }.getOrDefault(0L)
        if (current == version && geosite.length() == assetSiteLen && geoip.length() > 0) return true
