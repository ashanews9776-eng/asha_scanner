...
class GuardTileService : TileService() {
    ...
    private fun toggleConnection(): Boolean {
        ...
        if (isConnected) {
            // Disconnect
            startService(Intent(this, com.ahoura.asha_scanner_ip.core.vpn.AshaVpnService::class.java).setAction(GuardVpnService.ACTION_DISCONNECT))
        ...
        if (permissionIntent == null) {
            ...
            ContextCompat.startForegroundService(
                this,
                Intent(this, com.ahoura.asha_scanner_ip.core.vpn.AshaVpnService::class.java)
                    .setAction(GuardVpnService.ACTION_CONNECT)
                    .putExtra(GuardVpnService.EXTRA_CONFIG, config)
            )
        ...
        android.util.Log.w(LOG_TAG, "VPN permission required; opening the app to ask")
        openApp()
        return true
    }
    ...
    private fun collapseShade() {
        ...
        try {
            ...
            android.util.Log.w(LOG_TAG, "Could not collapse the shade: ${error.message}")
        }
    }
    ...
    private fun configJson(): String {
        ...
        return if (armed && picked == Protocol.PSIPHON.coreName) {
            CoreConfig.json(this, GuardVpnService.CHAIN_PROTOCOL_MARKER.lowercase())
        } else {
